...
public class StringUtil
{
	...
	public static string Read(string path)
	{
		...
		if (null == asset) {
			...
			if (null == asset) {
				Toolkit.Log("Did you omit the file extension?  Did you place the file in the Assets/Resources/ folder?  Path was " + path + " and without extension was " + basename);
			}
		}
		string text = asset.text;
		text = NormalizeLines(text);
		text = text.Trim();
		// Toolkit.Log("Toolkit.Read: " + text);
		return text;
	}
	...
	public static string[][] ParseCsv(string text, string fieldDelimiter = ",")
	{
		...
		// Toolkit.Log("Toolkit.ParseCsv: lines " + table.Length);
		return table;
	}
}
